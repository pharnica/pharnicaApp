...
const SpecificPharmacySelection = () => {
  ...
  const { selectedPharmacies, addSelectedPharmacy } = useOrderStore();
  ...
  const clusters = useMemo(() => {
    ...
    return supercluster.getClusters(
      ...
      Math.floor(Math.log2(360 / region.longitudeDelta)) // Calculate zoom level
    );
  }, [region, supercluster]);
  ...
  const handleSelectPharmacy = () => {
    if (selectedPharmacy) {
      addSelectedPharmacy(selectedPharmacy);
      router.back();
    }
  };
  ...
  return (
    <View style={tw`flex-1`}>
      ...
      {(!location || locating) && (
        <View
          style={tw`absolute bottom-4 right-4 bg-white p-3 rounded-full shadow-lg`}
        >
          <View style={tw`flex-row items-center`}>
            <ActivityIndicator  color="#22c55e" />
            <Text style={tw`ml-2 text-sm text-gray-600`}>
              {locating ? "Locating..." : "Getting your location..."}
            </Text>
          </View>
        </View>
      )}
      ...
      {selectedPharmacy && (
        <View
          style={tw`absolute bottom-16 p-4 left-4 right-4 bg-white rounded-xl shadow-md overflow-hidden`}
        >
          ...
          <View style={tw`flex-row items-center mt-1`}>
            ...
            >
              <Text
                style={tw`${
                  selectedPharmacy.isActive ? "text-green-600" : "text-red-600"
                } font-medium text-sm`}
              >
          ...
          <TouchableHighlight
            style={tw`mt-4 py-3 px-4 rounded-lg flex-row items-center justify-center ${
              selectedPharmacy.isActive ? "bg-green-500" : "bg-gray-200"
            }`}
            disabled={!selectedPharmacy.isActive}
            onPress={handleSelectPharmacy}
            underlayColor="#126b33"
          >
            <Text
              style={tw`font-bold mr-2  ${
                selectedPharmacy.isActive ? "text-white " : "text-gray-400"
              }`}
            >
              save and back
            </Text>
